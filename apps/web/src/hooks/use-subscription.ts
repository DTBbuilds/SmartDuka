...
const CACHE_TTL = {
  SUBSCRIPTION: 60 * 1000, // 60 seconds - longer cache to prevent UI flickering
  PLANS: 30 * 60 * 1000, // 30 minutes (plans rarely change)
  INVOICES: 60 * 1000, // 60 seconds - longer cache to prevent constant refetching
};
...
export function useSubscription() {
  ...
  // Debounced visibility refresh - only refresh if tab was hidden for > 30 seconds
  // This prevents aggressive refreshes that cause UI flickering
  const lastHiddenTimeRef = useRef<number | null>(null);
  const visibilityRefreshRef = useRef<NodeJS.Timeout | null>(null);
      ...
      if (document.visibilityState === 'hidden') {
        ...
        if (hiddenDuration > 30000) {
          ...
          if (visibilityRefreshRef.current) {
            clearTimeout(visibilityRefreshRef.current);
          }
          visibilityRefreshRef.current = setTimeout(() => {
            clearCache(CACHE_KEYS.SUBSCRIPTION);
  ...
  // Polling only for pending_payment status (active payment in progress)
  // Note: past_due and suspended are long-term states - don't poll for those
  useEffect(() => {
    if (!subscription) return;

    // Only poll for pending_payment (active STK push or payment processing)
    const isActivePayment = subscription.status === 'pending_payment';
    
    if (isActivePayment) {
      // Poll every 60 seconds for payment confirmation (reduced to prevent UI disruption)
  ...
  // Debounce ref to prevent refresh cascade from multiple events
  const subscriptionRefreshDebounceRef = useRef<NodeJS.Timeout | null>(null);
  ...
  // Debounced to prevent cascade when multiple events fire in quick succession
  useRefreshEvent(
    ['subscription:updated', 'subscription:created', 'subscription:reactivated', 'payment:completed'],
    () => {
      const now = Date.now();
      // Minimum 10 seconds between refreshes to prevent cascade and UI disruption
      if (now - lastSubscriptionRefreshRef.current < 10000) {
        return;
      }
      
      if (subscriptionRefreshDebounceRef.current) {
        clearTimeout(subscriptionRefreshDebounceRef.current);
      }
      
      subscriptionRefreshDebounceRef.current = setTimeout(() => {
...
export function useBillingHistory() {
  ...
  // Polling when there are pending invoices - reduced frequency to prevent refresh cascade
  useEffect(() => {
    if (pendingInvoices.length > 0) {
      // Poll every 60 seconds for pending invoice updates (reduced to prevent UI flickering)
      // Real-time updates come via WebSocket, this is just a fallback
      const interval = setInterval(() => {
  ...
  // Debounce ref to prevent refresh cascade from multiple events
  const billingRefreshDebounceRef = useRef<NodeJS.Timeout | null>(null);
  const lastBillingRefreshRef = useRef<number>(0);
  ...
  useRefreshEvent(
    ...
    () => {
      const now = Date.now();
      // Minimum 10 seconds between refreshes to prevent cascade and UI disruption
      if (now - lastBillingRefreshRef.current < 10000) {
        return;
      }
      
      if (billingRefreshDebounceRef.current) {
        clearTimeout(billingRefreshDebounceRef.current);
      }
      
      billingRefreshDebounceRef.current = setTimeout(() => {
        lastBillingRefreshRef.current = Date.now();
        clearCache(CACHE_KEYS.INVOICES);
        clearCache(CACHE_KEYS.PENDING_INVOICES);
        fetchHistory(20, 0, true);
      }, 1000); // Increased debounce delay

  ...
  const fetchBranches = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        } else if (!savedBranchId && branchList.length > 0) {
          // Auto-select on first login: prefer main branch, then first active branch
          const mainBranch = branchList.find((b: Branch) => b.type === 'main' && b.isActive);
          const firstActive = branchList.find((b: Branch) => b.isActive);
          const autoSelect = mainBranch || firstActive || branchList[0];
          if (autoSelect) {
            setCurrentBranchState(autoSelect);
            localStorage.setItem(BRANCH_STORAGE_KEY, autoSelect._id);
          }
        }
      } else {

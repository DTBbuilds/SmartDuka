...
function POSContent() {
      ...
      try {
        ...
        const payload = {
          ...
          payments: [
            {
              method: 'mpesa',
              amount: total - loyaltyPointsToRedeem, // Apply loyalty discount
              status: 'pending',
          ...
          customerPhone: selectedCustomer?.phone || phoneNumber,
  ...
  const handleMpesaSuccess = async (receiptNumber: string) => {
    ...
    const mpesaLoyaltyBalance = selectedCustomer?.loyalty
      ? selectedCustomer.loyalty.availablePoints - loyaltyPointsToRedeem
      : undefined;
    const receipt: ReceiptData = {
      ...
      loyaltyPointsBalance: mpesaLoyaltyBalance,
    };
  ...
  const handleConfirmPayment = async () => {
    try {
      ...
      // Apply loyalty discount to payment amount
      ...
      const payload = {
        ...
        customerId: selectedCustomer?._id || undefined,
        customerName: selectedCustomer?.name || customerName || undefined,
        customerPhone: selectedCustomer?.phone || customerName || undefined,
        loyaltyPointsToRedeem: loyaltyPointsToRedeem > 0 ? loyaltyPointsToRedeem : undefined,
        cashierId,
        cashierName,
        shiftId: currentShift?._id,
      ...
      const loyaltyBalance = selectedCustomer?.loyalty
        ? (selectedCustomer.loyalty.availablePoints - loyaltyPointsToRedeem + loyaltyEarned)
        : undefined;
      const receipt: ReceiptData = {
        ...
        <div className="grid gap-2 lg:gap-4 grid-cols-1 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_420px] h-[calc(100vh-140px)] lg:h-[calc(100vh-100px)]">
          ...
          <section className="hidden lg:flex flex-col overflow-y-auto h-full gap-1">
            ...
            <POSCartCompact
              ...
              onPointsRedemption={setLoyaltyPointsToRedeem}

        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Revenue, cost, and profit over the last 30 days</CardDescription>
            </CardHeader>
            <CardContent>
              {profitData.dailyTrend.every(d => d.revenue === 0) ? (
                ...
                <div className="h-64 flex items-end gap-1">
                  {profitData.dailyTrend.slice(-14).map((day, i) => {
                    const maxRevenue = Math.max(...profitData.dailyTrend.map(d => d.revenue), 1);
                    const revenueHeight = (day.revenue / maxRevenue) * 100;
                    const profitHeight = day.revenue > 0 ? (day.profit / day.revenue) * revenueHeight : 0;
                    
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <div className="w-full relative" style={{ height: `${revenueHeight || 2}%`, minHeight: '4px' }}>
                          {/* Revenue bar (full) */}
                          <div 
                            className="absolute inset-0 bg-blue-200 dark:bg-blue-900 rounded-t"
                            title={`Revenue: ${formatCurrency(day.revenue)}`}
                          />
                          {/* Profit overlay */}
                          <div 
                            className={`absolute bottom-0 left-0 right-0 rounded-t ${day.profit >= 0 ? 'bg-green-500' : 'bg-red-500'}`}
                            style={{ height: `${Math.abs(profitHeight)}%`, minHeight: day.profit !== 0 ? '2px' : '0' }}
          ...
          <Card>
            ...
            <CardContent>
              <div className="h-32 flex items-end gap-1">
                {profitData.dailyTrend.slice(-14).map((day, i) => {
                  const height = Math.min(Math.max(day.margin, 0), 100);
                  const color = day.margin >= 30 ? 'bg-green-500' : 
                               day.margin >= 20 ? 'bg-yellow-500' : 
                               day.margin >= 10 ? 'bg-orange-500' : 'bg-red-500';
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <div 
                        ...
                        title={`${day.date}: ${day.margin}% margin`}
                      />
                      <span className="text-[10px] text-muted-foreground">
                        {new Date(day.date).getDate()}
                      </span>

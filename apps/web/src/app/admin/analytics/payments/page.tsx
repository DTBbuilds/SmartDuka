          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="p-4 pb-2">
                <CardTitle className="text-sm font-semibold">Payment Trend</CardTitle>
                <CardDescription className="text-xs">Last 14 days</CardDescription>
              </CardHeader>
              <CardContent className="p-2 md:p-4 pt-0">
                {stats.dailyTrend.length === 0 || stats.dailyTrend.every(d => d.total === 0) ? (
                  <div className="h-56 flex items-center justify-center text-muted-foreground">
                    <p className="text-sm">No payment data for this period</p>
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height={240}>
                    <AreaChart data={stats.dailyTrend.map(d => ({ ...d, label: `${new Date(d.date).getDate()}/${new Date(d.date).getMonth() + 1}` }))} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                      <defs>
                      ...
                      <Tooltip
                        ...
                        formatter={((value: any) => [`Ksh ${(value ?? 0).toLocaleString()}`, 'Amount']) as any}
                      />
                      <Area type="monotone" dataKey="total" stroke="#22c55e" strokeWidth={2.5} fill="url(#paymentGradient)" dot={{ r: 3, fill: '#22c55e', strokeWidth: 0 }} activeDot={{ r: 5, fill: '#22c55e', strokeWidth: 2, stroke: '#fff' }} />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
                  ...
                  <>
                    <div className="flex justify-center mb-4">
                      <ResponsiveContainer width={140} height={140}>
                        <PieChart>
                          <Pie data={stats.methodBreakdown} dataKey="total" nameKey="method" cx="50%" cy="50%" innerRadius={40} outerRadius={65} strokeWidth={2} stroke="hsl(var(--background))">
                            {stats.methodBreakdown.map((_, idx) => (
                        ...
                        return (
                          <div key={i} className="flex items-center justify-between text-sm">
                            ...
                            <div className="text-right">
                              <span className="text-xs font-semibold">Ksh {method.total.toLocaleString()}</span>
                              <span className="text-xs text-muted-foreground ml-1.5">({method.percentage}%)</span>
                            </div>
                          </div>
                        );
                      })}

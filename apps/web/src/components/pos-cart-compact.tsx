...
import { ShoppingCart, Trash2, User, Gift, Package } from 'lucide-react';
...
export function POSCartCompact({
  ...
  return (
    <div className="w-full flex flex-col overflow-hidden flex-shrink-0">
        ...
        <div className="flex-shrink-0 h-[120px] overflow-y-auto border-b px-2 py-1">
          <div className="space-y-0.5">
            {cartItems.map((item) => (
              <div
                key={item.productId}
                className="flex items-center justify-between text-xs p-1 hover:bg-slate-50 dark:hover:bg-slate-900 rounded group gap-2"
              >
                {item.image ? (
                  <img src={item.image} alt={item.name} className="w-7 h-7 rounded object-cover flex-shrink-0 bg-muted" loading="lazy" />
                ) : (
      ...
      {/* Customer Selection with Loyalty - Only when items exist */}
      ...
      {loyaltyPointsToRedeem > 0 && (
        <div className="flex-shrink-0 px-2 py-1 bg-emerald-50 dark:bg-emerald-950/30 border-b">
      ...
      {/* Fallback Customer Info - Only if no customer select handler */}
      {cartItems.length > 0 && !onSelectCustomer && (
      ...
      {cartItems.length > 0 && (
        <div className="flex-shrink-0 px-2 py-2">
          ...
          >
            {isCheckingOut ? 'Processing…' : `Checkout • ${formatCurrency(finalTotal)}`}
          </Button>
        </div>
      )}
    </div>

...
import { ShoppingCart, Trash2, User, X, Minus, Plus, ChevronUp, ChevronDown, Gift, Package } from 'lucide-react';
import { CustomerSelect } from './pos/customer-select';
import { CustomerWithLoyalty } from '@/hooks/use-customers';
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 lg:hidden bg-background rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col animate-in slide-in-from-bottom duration-300">
            ...
            <div className="space-y-3">
              {cartItems.map((item) => {
                ...
                >
                  {/* Product Image */}
                  {item.image ? (
                    <img src={item.image} alt={item.name} className="w-11 h-11 rounded-lg object-cover flex-shrink-0 bg-muted" loading="lazy" />
                  ) : (
        ...
        {cartItems.length > 0 && onSelectCustomer && (
          <div className="px-4 py-3 border-t">
            <CustomerSelect
              selectedCustomer={selectedCustomer || null}
              onSelectCustomer={onSelectCustomer}
              onPointsRedemption={onPointsRedemption}
              cartTotal={total}
              className="w-full"
            />
          </div>
        ...
        {loyaltyPointsToRedeem > 0 && (
          <div className="px-4 py-2 bg-emerald-50 dark:bg-emerald-950/30 border-t">
            <div className="flex items-center justify-between text-sm">
              <span className="text-emerald-700 dark:text-emerald-300 flex items-center gap-1">
                <Gift className="h-3.5 w-3.5" />
                Loyalty Discount
              </span>
              <span className="font-medium text-emerald-600">-{formatCurrency(loyaltyPointsToRedeem)}</span>
            </div>
          </div>
        )}

        {/* Fallback Customer Name Input - only when no CustomerSelect handler */}
        {cartItems.length > 0 && !onSelectCustomer && showCustomerInput && (
          <div className="px-4 py-3 border-t">
            <div className="flex items-center gap-2">
        ...
        {cartItems.length > 0 && (
          <div className="border-t px-4 py-4 space-y-3 bg-slate-50 dark:bg-slate-900/50">
            {/* Customer toggle - only when no CustomerSelect handler */}
            {!onSelectCustomer && !showCustomerInput && (
              <button
            ...
            <div className="space-y-1.5 text-sm">
              ...
              {loyaltyPointsToRedeem > 0 && (
                <div className="flex justify-between text-emerald-600">
                  ...
                  <span>-{formatCurrency(loyaltyPointsToRedeem)}</span>
                </div>
              )}
              <div className="flex justify-between font-bold text-lg pt-2 border-t">
                <span>Total</span>
                <span className="text-primary">{formatCurrency(total - loyaltyPointsToRedeem)}</span>
              </div>
            </div>
            ...
            >
              ...
              {isCheckingOut ? 'Processing...' : `Pay ${formatCurrency(total - loyaltyPointsToRedeem)}`}
            </Button>
          </div>
        )}
      </div>

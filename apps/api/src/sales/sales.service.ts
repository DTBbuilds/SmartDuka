...
export class SalesService {
  ...
  async checkout(shopId: string, userId: string, branchId: string, dto: CheckoutDto): Promise<OrderDocument> {
    ...
    if (dto.customerId && dto.loyaltyPointsToRedeem && dto.loyaltyPointsToRedeem > 0) {
      try {
        ...
        if (redemptionResult) {
          ...
          this.logger.log(`Redeemed ${loyaltyPointsRedeemed} loyalty points for customer ${dto.customerId}`);
        }
      } catch (error: any) {
    ...
    if (dto.customerId && paymentStatus === 'paid') {
      try {
        ...
        if (earnResult) {
          ...
          const lastTxn = earnResult.transactions?.[earnResult.transactions.length - 1];
          loyaltyPointsEarned = lastTxn?.type === 'earn' ? lastTxn.amount : 0;
          order.loyaltyPointsEarned = loyaltyPointsEarned;
          await order.save();
          this.logger.log(`Awarded ${loyaltyPointsEarned} loyalty points to customer ${dto.customerId}`);
        }
      } catch (error: any) {
      ...
      try {
        await this.customersService.updatePurchaseStats(dto.customerId, finalTotal);
      } catch (error: any) {
        this.logger.warn(`Failed to update customer purchase stats: ${error?.message}`);
      }
    }
  ...
  async getSalesAnalytics(shopId: string, range: string = 'month', branchId?: string) {
    ...
    const averageOrderValue = periodOrderCount > 0 ? Math.round(periodRevenue / periodOrderCount) : 0;

    // Growth calculations
    const revenueGrowth = prevRevenue === 0 ? (periodRevenue > 0 ? 100 : 0) : Math.round(((periodRevenue - prevRevenue) / prevRevenue) * 1000) / 10;
    const ordersGrowth = prevOrderCount === 0 ? (periodOrderCount > 0 ? 100 : 0) : Math.round(((periodOrderCount - prevOrderCount) / prevOrderCount) * 1000) / 10;

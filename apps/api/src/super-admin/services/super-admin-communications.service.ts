    ...
    for (const batch of batches) {
      const batchResults = await Promise.allSettled(
        batch.map(async (recipient) => {
          ...
          // Wrap content in SmartDuka branded layout and send
          const wrappedHtml = this.emailService.wrapInLayout(dto.htmlContent);
          ...
          return { recipient, emailLog, sendResult };
        }),
      );
      ...
      for (const batchResult of batchResults) {
        if (batchResult.status === 'fulfilled') {
          const { recipient, emailLog, sendResult } = batchResult.value;
          ...
          result.errors.push({ email: 'unknown', error: error?.message || 'Batch processing error' });

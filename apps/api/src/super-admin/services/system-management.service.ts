...
export class SystemManagementService {
  ...
  async getDashboardStats(): Promise<{
    ...
    // Use $facet aggregations for single-query performance (much faster than multiple countDocuments)
    const [shopStats, subscriptionStats, paymentStats, userStats, auditStats, emailStats] = await Promise.all([
      // Shop stats - single aggregation
      ...
      // Subscription stats - single aggregation
      this.subscriptionModel.aggregate([
        {
          $facet: {
            total: [{ $count: 'count' }],
            active: [{ $match: { status: 'active' } }, { $count: 'count' }],
            trial: [{ $match: { status: 'trial' } }, { $count: 'count' }],
            pastDue: [{ $match: { status: 'past_due' } }, { $count: 'count' }],
            suspended: [{ $match: { status: 'suspended' } }, { $count: 'count' }],
      ...
      // Payment stats - single aggregation
    ...
    const totalShops = shops.total?.[0]?.count || 0;
    const activeShops = shops.active?.[0]?.count || 0;
    const pendingShops = shops.pending?.[0]?.count || 0;
    const suspendedShops = shops.suspended?.[0]?.count || 0;
    const flaggedShops = shops.flagged?.[0]?.count || 0;
    const newShopsThisMonth = shops.newThisMonth?.[0]?.count || 0;

    const totalSubscriptions = subs.total?.[0]?.count || 0;
    const activeSubscriptions = subs.active?.[0]?.count || 0;
    const trialSubscriptions = subs.trial?.[0]?.count || 0;
    const pastDueSubscriptions = subs.pastDue?.[0]?.count || 0;
    const suspendedSubscriptions = subs.suspended?.[0]?.count || 0;
    const expiredSubscriptions = subs.expired?.[0]?.count || 0;
    const mrr = subs.mrr?.[0]?.total || 0;

    const totalRevenue = payments.totalRevenue?.[0]?.total || 0;
    const revenueThisMonth = payments.revenueThisMonth?.[0]?.total || 0;
    const pendingInvoicesCount = payments.pendingInvoices?.[0]?.count || 0;
    const pendingAmount = payments.pendingInvoices?.[0]?.total || 0;
    const failedPayments = payments.failedPayments?.[0]?.count || 0;

    const totalUsers = users.total?.[0]?.count || 0;
    const adminUsers = users.admins?.[0]?.count || 0;
    const activeUsersToday = users.activeToday?.[0]?.count || 0;

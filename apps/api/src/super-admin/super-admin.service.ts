...
export class SuperAdminService {
   ...
   */
  async getPendingShops(limit: number = 50, skip: number = 0): Promise<any[]> {
    return this.shopModel
      .find({ status: 'pending' })
      .select('name email phone shopId status city businessType createdAt isFlagged subscriptionPlan')
      .sort({ createdAt: -1 })
      .limit(limit)
      .skip(skip)
      .lean()
   ...
   */
  async getVerifiedShops(limit: number = 50, skip: number = 0): Promise<any[]> {
    return this.shopModel
   ...
   */
  async getActiveShops(limit: number = 50, skip: number = 0): Promise<any[]> {
    return this.shopModel
      .find({ status: 'active' })
      .select('name email phone shopId status city businessType createdAt isFlagged subscriptionPlan subscriptionStatus')
      .sort({ createdAt: -1 })
      .limit(limit)
      .skip(skip)
      .lean()
   ...
   */
  async getSuspendedShops(limit: number = 50, skip: number = 0): Promise<any[]> {
    return this.shopModel
      .find({ status: 'suspended' })
      .select('name email phone shopId status city businessType createdAt isFlagged suspensionDate suspensionReason')
      .sort({ createdAt: -1 })
      .limit(limit)
      .skip(skip)
      .lean()
   ...
   */
  async getFlaggedShops(limit: number = 50, skip: number = 0): Promise<any[]> {
    return this.shopModel
      .find({ isFlagged: true })
      .select('name email phone shopId status city businessType createdAt isFlagged flagReason')
      .sort({ createdAt: -1 })
      .limit(limit)
      .skip(skip)
      .lean()
      ...
      try {
        await this.emailService.sendTemplateEmail({
          ...
          variables: {
            ...
            loginUrl: `${process.env.FRONTEND_URL || 'https://www.smartduka.org'}/login`,
            dashboardUrl: `${process.env.FRONTEND_URL || 'https://www.smartduka.org'}/admin`,
          },
        });
  ...
  /**
   * Get all shop counts in a single optimized aggregation query
   * Uses caching for better performance
   */
  async getAllShopCounts(): Promise<{
    pending: number;
    verified: number;
    active: number;
    suspended: number;
    ...
    const cacheKey = ADMIN_CACHE_KEYS.shopStats();
    ...
    // Single aggregation for all counts - much faster than multiple countDocuments
    ...
    if (this.cacheService) {
      this.cacheService.set(cacheKey, counts, CACHE_TTL.ADMIN_COUNTS);
    }
  ...
  async getPendingShopsCount(): Promise<number> {
    const counts = await this.getAllShopCounts();
  ...
  /**
   * Invalidate shop-related caches (call after shop status changes)
   */
  invalidateShopCache(): void {
    if (this.cacheService) {
      this.cacheService.deletePattern('admin:shops:*');
    }
  }

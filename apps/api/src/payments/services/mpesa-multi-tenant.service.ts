...
export class MpesaMultiTenantService {
  ...
  async getShopMpesaConfig(shopId: string): Promise<{
    ...
    // Decrypt credentials
    const decryptedConfig = this.encryptionService.decryptMpesaConfig(
      shop.mpesaConfig!,
    );
    ...
    return {
      ...
      callbackUrl:
        shop.mpesaConfig!.callbackUrl || this.getDefaultCallbackUrl(),
  ...
  async getMpesaConfigStatus(shopId: string): Promise<{
    ...
    if (!shopId.match(/^[0-9a-fA-F]{24}$/)) {
      this.logger.warn(
        `getMpesaConfigStatus called with invalid shopId format: ${shopId}`,
      );
      return {
        isConfigured: false,
        isVerified: false,
    ...
    if (!hasCredentials) {
      message =
        'M-Pesa not configured. Please add your Paybill/Till credentials to accept mobile payments.';
    } else if (!isEnabled) {
      message =
        'M-Pesa credentials saved but not enabled. Enable M-Pesa in settings to accept payments.';
    } else if (!isVerified) {
      message =
        'M-Pesa credentials pending verification. Please verify your credentials.';
    } else {
   ...
   */
  async getShopsWithoutMpesaConfig(): Promise<
    { shopId: string; shopName: string; email: string; createdAt: Date }[]
  > {
  ...
  async initiateSTKPush(params: {
    ...
    const { shopId, phoneNumber, amount, orderId, orderNumber, description } =
      params;
    ...
    try {
      ...
      this.logger.log(
        `Initiating STK Push for shop ${shopId}, order ${orderNumber}, amount ${amount}`,
      );
      ...
      if (data.ResponseCode === '0') {
        // Create transaction record — orderNumber/idempotencyKey/cashierId are
        // schema-required; without them the save fails AFTER Daraja has sent
        // the STK prompt, orphaning the payment (no record for the callback
        // to converge).
        const transaction = new this.transactionModel({
          ...
          cashierId: params.cashierId
            ? new Types.ObjectId(params.cashierId)
            : undefined,
  ...
  async handleCallback(
    ...
    try {
      ...
      // CONVERGENCE: a provider-confirmed payment must record a
      // PaymentTransaction (idempotent on CheckoutRequestID) which in turn
      // converges the canonical order's payment/lifecycle state. Failures
      // here must not fail the callback — Daraja retries on non-zero acks.
      if (resultCode === 0) {
        if (!transaction.cashierId) {
          this.logger.warn(
            `Confirmed payment for ${checkoutRequestId} lacks cashier attribution - payment recorded on transaction only, order convergence requires manual reconciliation`,
          );
        } else {
          try {
            ...
            this.logger.error(
              `Failed to record payment/convergence for ${checkoutRequestId}: ${error?.message}`,
            );
          }
        }
      }
  ...
  async updateShopMpesaConfig(
    ...
    if (config.consumerSecret) {
      ...
      updateData['mpesaConfig.consumerSecretIv'] =
        encryptedConfig.consumerSecretIv;
      updateData['mpesaConfig.consumerSecretTag'] =
        encryptedConfig.consumerSecretTag;
    }
  ...
  async verifyShopMpesaCredentials(shopId: string): Promise<{
    ...
    try {
      ...
      await this.getAccessToken(
        decryptedConfig.consumerKey,
        decryptedConfig.consumerSecret,
      );
